...
if __name__ == '__main__':
    ...
    #     x_band_data['mean'].values, ds=raster_list[0], returnma=True
    # )
    #
    # extent = geolib.ds_geom_extent(raster_list[0])
    #
    # plt.imshow(shade, extent=extent, cmap='gray', clim=(1, 255))
    # plt.imshow(x_band_data['mean'], alpha=0.4, extent=extent)
    #
    # plt.show()

    # xr.apply_ufunc(ols,
    #                x_band_data.groupby('time'),
    #                input_core_dims=[['lat'], ['lon']],
    #                output_dtypes=[float])

    x_band_data.groupby('time').apply(ols)